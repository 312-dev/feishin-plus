...
export function useMusicVideoSync(
    ...
    // The track the panel is on right now, readable from callbacks that outlive the effect that
    // created them. See the retry below.
    const cacheKeyRef = useRef<null | string>(null);
    ...
    useEffect(() => {
        ...
        const disqualifyAndRetry = () => {
            ...
            // Not gated on `destroyed`. Writing the cleared candidate above changes `match`,
            // which tears this effect down immediately, so `destroyed` is true for the whole
            // retry - and gating on it meant the progress caption stopped updating and, once the
            // candidates ran out, `isLoading` was never cleared and the panel span forever
            // instead of falling back. What matters is whether the track has moved on, which is
            // what `cacheKeyRef` answers.
            const startedFor = cacheKey;
            const isCurrent = () => cacheKeyRef.current === startedFor;

            retryMusicVideoMatch(
                song,
                match.remainingCandidateIds ?? [],
                match.candidateCount ?? (match.remainingCandidateIds?.length ?? 0) + 1,
                (status) => {
                    if (isCurrent()) setLoadingStatus(status);
                },
            ).finally(() => {
                if (isCurrent()) {
                    setIsLoading(false);
        ...
        const onVideoError = () => {
            // The element's own `error` is the only thing that says why a file that is on disk and
            // decodes fine everywhere else would not play, so it is worth recording rather than
            // just noting that something went wrong.
            logger.warn('Music video: local playback error', {
