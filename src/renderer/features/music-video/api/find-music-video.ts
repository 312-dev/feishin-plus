...
export async function findMusicVideo(
    ...
    await scoreAndStore(song, cacheKey, videoIds, videoIds.length, onProgress);
}
...
export async function retryMusicVideoMatch(
    ...
    await scoreAndStore(song, cacheKey, remainingCandidateIds, candidateCount, onProgress);
}
...
async function scoreAndStore(
    ...
    try {
        ...
        for (const [index, videoId] of videoIds.entries()) {
            // Counted against the original list, not what is left of it: `videoIds` shrinks with
            // every disqualification, so numbering within it restarted at one each time.
            const attempt = candidateCount - videoIds.length + index + 1;
            onProgress?.(`Checking candidate ${attempt} of ${candidateCount}...`);
